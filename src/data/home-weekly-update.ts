export interface HomeWeeklyUpdate {
  heading: string;
  body: string[];
}

export const homeWeeklyUpdate: HomeWeeklyUpdate = {
  heading: "What's Dave working on this week?",
  body: [
    "This week I'm working on finishing my MKVI Canberra Bomber. This version has a larger body and wing cord. I've also made nicer undercarriage for it and fitted new 30mm EDF's. The new receiver with differential brushed ESC's blew up after a day, so I've ordered another one.",
    "I'm also learning how to fly my MKVI Chaotic 3D flyer in the park and at indoor aviation. This little number is a lot of fun.",
    'Check out my construction and flight videos for these two planes.',
  ],
};

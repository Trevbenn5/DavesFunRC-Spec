...
- A safe git branch name not already taken (e.g. new-contact-form)
...
3. `branch_name`  
   - Format: `features/<feature_slug>`  
   - Example: `features/card-component`.
...
Before making any content, switch to a new Git branch using the `branch_name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `features/card-component-01`
